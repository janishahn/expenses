...
| Dashboard metrics, quick periods, tag-only secondary-filter panel and scoped drill-downs, charts, privacy, category focus, quick add, scheduled-tag defaults and removal | `dashboard.spec.ts`, `tag-filters.spec.ts`, `visual.spec.ts` | `dashboard.mobile.spec.ts`, `tag-filters.mobile.spec.ts`, `summaries.mobile.spec.ts`, `visual.mobile.spec.ts` | Yes |
