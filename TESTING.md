...
| Dashboard metrics, quick periods, shared secondary-filter panel and tag scopes, charts, privacy, category focus, quick add, scheduled-tag defaults and removal | `dashboard.spec.ts`, `tag-filters.spec.ts`, `visual.spec.ts` | `dashboard.mobile.spec.ts`, `tag-filters.mobile.spec.ts`, `visual.mobile.spec.ts` | Yes |
