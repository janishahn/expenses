...
| Dashboard metrics, periods, unified tag filtering, charts, privacy, category focus, quick add, scheduled-tag defaults and removal | `dashboard.spec.ts`, `tag-filters.spec.ts`, `visual.spec.ts` | `dashboard.mobile.spec.ts`, `tag-filters.mobile.spec.ts`, `visual.mobile.spec.ts` | Yes |
...
| Transaction actions, unified multi-tag filtering, search reveal, selection, detail, edit, deletion, attachments, location, durable tracking | `transactions.spec.ts`, `tag-filters.spec.ts`, `transactions-detail.spec.ts`, `transactions-deletion.spec.ts`, `transactions-attachments.spec.ts`, `navigation.desktop.spec.ts`, `focus-management.spec.ts` | `transactions.mobile.spec.ts`, `tag-filters.mobile.spec.ts`, `focus-management.mobile.spec.ts`, `interaction-audit.mobile.spec.ts` | Yes |
...
| Insights filters and unified multi-tag scopes, charts, Net view selection/data view, and drill-through | `insights.spec.ts`, `tag-filters.spec.ts` | `insights.mobile.spec.ts`, `tag-filters.mobile.spec.ts`, `interaction-audit.mobile.spec.ts` | Yes |
