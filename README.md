...
Beyond the pages above, Expenses includes Forecast and What-If scenarios for projecting cash flow forward, a Digest summary of recent activity, and exportable PDF reports. Reconcile processes a bank statement CSV into one inbox of unresolved rows. You can accept a suggested transaction, choose another likely match, or create and edit a transaction without leaving the page; Commerzbank CSV is the only supported format for now. The native iOS app covers the same core flows, including setup and login, dashboard, transactions, budgets, insights, planning, reports, reconciliation, receipts, and the spending assistant.
