...
*Screenshots show the web app's financial-switchboard interface in light mode with `uv run mock-db` sample data. Desktop uses a fully visible grouped sidebar, aligns its page action to the centered content canvas, and keeps that canvas stationary while dialogs open; mobile opens every workspace from a leading Menu beside the page title and keeps the current page's primary create action in a shared lower-right button. Theme selection lives in Settings on both layouts.*

### Dashboard

![Dashboard](docs/screenshots/dashboard-light.png)

The dashboard answers "where do I stand right now". Pick a period (this month, last month, all time, or a custom range) and see the available balance with actual history and a distinct projected continuation, followed by income, spending, net movement, and relevant budget health. The frequently used period switch stays visible, while one quiet Filters action holds occasional transaction-type and tag scoping; both controls share the same height wherever they appear together. The desktop filter panel leaves document scrolling available so opening it never moves the centered page canvas. Tags can either show transactions carrying any of several selected tags or exclude transactions carrying any selected tag; the filter badge counts active filter kinds, and compact chips keep the applied scope visible. Available balance, balance history, and budgets remain based on all activity. Balance history begins with the first known snapshot instead of inventing an opening balance for earlier months. An overall monthly budget shows plan pace; category-only planning shows the aggregate category status plus the category that most needs attention. When no budgets exist, the planning lane disappears instead of prompting for an unused feature: desktop uses three equal metric columns, while mobile keeps income and spending side by side with net movement across the full second row. Hovering the desktop balance path reveals exact actual or likely values. Privacy mode conceals headline and analytical values while keeping budget health and recent transaction amounts readable. Mobile omits the history chart and keeps transaction type in the filter sheet instead of adding another permanent control. Recent transactions use the available desktop panel height without clipping a partial row or creating a nested scroller; mobile keeps the four latest rows. An accessible six-month category-band view shows how the composition of spending changed over time. Category donut legends use aligned responsive columns so labels and amounts remain easy to scan without nested scrolling, while segment tooltips show only the hovered percentage.

### Transactions

![Transactions ledger](docs/screenshots/transactions-light.png)

Transactions is the full working ledger and the page you will spend the most time in. Search stays directly available as the leftmost header action, while Inbox, Trash, and Export CSV share one overflow menu. On mobile, that overflow sits at the top-right beside the page title; the compact row below contains only Search and Filters. Opening this lightweight menu leaves the page scrollbar available. Search expands its trigger into a search field in place: on desktop the field grows leftward out of the trigger without shifting the surrounding controls, and on mobile it takes over the control row and pushes Filters off the row's edge until search closes. It uses typo-tolerant matching across titles and descriptions while preserving chronological order. Desktop keeps the quick period switch visible in the shared scope header. On mobile, period joins type, category, and tags inside the focused filter sheet so the four range labels never compress or overlap; all of those choices use the same staged Apply/Cancel flow. Tag mode switches between showing transactions carrying any selected tag and excluding transactions carrying any selected tag. Applied filter kinds appear as removable chips and scope the list, summary, CSV export, and “all filtered” bulk actions together. Transaction checkboxes are always available, and the stable register header changes in place to expose a segmented bulk scope and bulk editing after you select a row instead of requiring a separate selection mode. Every entry supports tags, a category, receipt attachments, and an optional location. A tag can also define one inclusive date range: new manual transactions in that range show it as a removable default on web and iOS. A trash with soft delete keeps a mistaken delete recoverable. You can export the current view to CSV at any time, or download a self-describing portable archive from Settings when you need a fuller machine-readable export for migrations and agents.
...
Insights is the visual analysis board. It charts income against expenses over the last twelve months, spending composition, the trend for any selected category, top categories, and budget versus actual for a chosen month. Analysis and Net are URL-backed page tabs, while the quick period switch and Filters action use the same responsive header placement as Dashboard and Transactions. Switching between the differently sized views keeps the desktop page canvas anchored. Filters holds occasional transaction-type and multi-tag scopes. Tags can either analyze transactions carrying any of several selected tags or exclude transactions carrying any selected tag, and the scope carries into the Net view and transaction drill-downs. The category-trend choice and budget month sit beside the sections they configure because they do not filter the whole page. Budget figures stay based on full budget activity. A single selected month renders explicit income and expense points instead of an empty-looking axis. The Net view shows how recorded income raises the balance, spending lowers it, and the last bar reaches the period result. Large categories stay explicit and small categories group under Other. On web, the data view keeps every category available for drill-down. Mobile web and the native iOS app use horizontal chart rows that fit normal vertical page scrolling.

### Recurring income and expenses

![Recurring rules](docs/screenshots/recurring-light.png)

Recurring rules model the fixed parts of your finances: salary, rent, subscriptions, and anything else that repeats on a schedule. URL-backed Commitments and Audit tabs switch the whole workspace without looking like value filters. Add rule opens a focused creation modal, and each row's edit action opens the same workflow with that rule loaded. Each rule can auto-post its transaction when it comes due, and the audit view records what was posted. The summary cards frame recurring income against committed recurring costs as a coverage ratio, so you can see how much of each month is already accounted for before any discretionary spending.

Templates, categorization rules, categories, and tags use the same focused editing model: their page-level Add action opens a modal, and compact row actions open updates without keeping a form permanently beside the library. Templates can be reordered directly by their drag handles, with keyboard reordering available from the focused handle. Tags keep creation and merging as two separate modal workflows; the tag detail page shows settings as a read-only summary until Edit opens its focused editor. Tag creation or settings can enable an automatic date range for events such as vacations. Archiving a finished tag preserves its settings and every existing transaction association while removing it from future assignment pickers; restore makes it assignable again, and hard delete remains available when history should actually be removed. Archived tags remain useful in historical filters unless their separate Hide from filter menus setting is enabled.
...
Beyond the pages above, Expenses includes Forecast and What-If scenarios for projecting cash flow forward, a Digest summary of recent activity, and exportable PDF reports that can include only selected tags or exclude selected tags alongside their existing type and category scope. Reconcile processes a bank statement CSV into one inbox of unresolved rows. You can accept a suggested transaction, choose another likely match, mark a known discrepancy reviewed, or create and edit a transaction without leaving the page; Commerzbank CSV is the only supported format for now. The native iOS app covers the same core flows, including setup and login, dashboard, transactions, budgets, insights, planning, reports, reconciliation, receipts, and the spending assistant.
...
A successful call returns `201` with the created transaction, including its resolved `category` and a `location_status` of `not_provided`, `stored`, or `ignored_partial`. The endpoint also silently attaches every active tag whose inclusive automatic date range contains the transaction date. This automatic behavior applies only to `/api/ingest`; manual forms show the same defaults before saving, while recurring posts, reconciliation, and imports do not add them. Missing or invalid tokens return `401`; an unknown category returns `404`; an ambiguous category returns `409`.
