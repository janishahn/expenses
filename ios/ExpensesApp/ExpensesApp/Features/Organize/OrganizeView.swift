    ...
    var body: some View {
        List {
            if model.identity?.authenticated != true {
                ...
                CategoryList(categories: categories.categories.filter { $0.archivedAt == nil }) { category in
                ...
                CategoryArchiveLink(archivedCount: categories.categories.filter { $0.archivedAt != nil }.count)
            } else if model.isLoading {
                LoadingStateSection(title: "Loading categories")
            } else {
                CategoryList(categories: []) { category in
                    activeSheet = .category(category)
                } onArchiveToggle: { category in
                    Task {
                        await model.setCategoryArchived(category, archived: category.archivedAt == nil)
                ...
                CategoryArchiveLink(archivedCount: 0)
            }
        }
        ...
        .animation(.easeInOut(duration: 0.18), value: model.isLoading && model.categories == nil)
    ...
    var body: some View {
        List {
            if model.identity?.authenticated != true {
                ...
                LoadingStateSection(title: "Loading rules")
            } else {
                RuleList(rules: []) { rule in
                    activeSheet = .rule(rule)
                } onToggle: { rule in
                    Task {
                        await model.toggleRule(rule, enabled: !rule.enabled)
        ...
        .animation(.easeInOut(duration: 0.18), value: model.isLoading && model.rules == nil)
        .onChange(of: quickAddTrigger) { _, _ in
            activeSheet = .rule(nil)
        }
    }
