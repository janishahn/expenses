...
| S-13 | Tools | DiagnosticsView (+ ErrorDetailsView) | Features/Diagnostics/DiagnosticsView.swift:3 | backend URL field; test connection; reset to local; read-only backend/session; pull-refresh; loading overlay | loading, success, error | none (ungated) | Audited | F-018(deferred) | 2026-06-30 |
| S-14 | Account | AuthView (signed-in + signed-out) | Features/Auth/AuthView.swift:5 | OUT: username/password/setup-token/device fields; login/setup/signup. IN: identity rows; logout; ingest token create/rotate/revoke/copy; CSV import; balance snapshots CRUD; mobile sessions revoke; appearance picker; pull-refresh | loading, success, empty(token), partial(result), error(single), unauthenticated | authenticated (in/out branch) | Fixed | F-016, F-019(deferred) | 2026-06-30 |
