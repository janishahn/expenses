...
Tags with an automatic date range show a compact Active, Upcoming, or Ended status in the tag library and their inclusive dates in the row evidence. In manual transaction creation, an active automatic tag uses the normal selected chip with a small `Auto` label and the same visible remove action as any other tag. The label explains the source without creating a second tag control or hiding the final saved selection. Transaction editing never introduces an automatic default.
